...
// File: contracts/interfaces/IPippiPair.sol
...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public override constant name = 'Pancake LPs';
    string public override constant symbol = 'Cake-LP';
...
contract PippiPair is IPippiPair, PancakeERC20 {
    using SafeMath  for uint;
...
// File: contracts/PippiFactory.sol

pragma solidity =0.6.8;



contract PippiFactory is IPancakeFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(PippiPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        bytes memory bytecode = type(PippiPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IPippiPair(pair).initialize(token0, token1);
