...
// File: contracts/libs/token/HRC20/IHRC20.sol
...
// File: contracts/libs/token/HRC20/SafeHRC20.sol
...
// File: contracts/libs/token/HRC20/HRC20.sol
...
// File: contracts/PipiBar.sol

pragma solidity 0.6.8;



// PipiBar with Governance.
contract PipiBar is HRC20('Little Pipi Shrimp Token', 'xPIPI') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // Safe pipi transfer function, just in case if rounding error causes pool to not have enough PIPIs.
    function safePipiTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 pipiBal = pipi.balanceOf(address(this));
        if (_amount > pipiBal) {
            pipi.transfer(_to, pipiBal);
        } else {
            pipi.transfer(_to, _amount);
        }
    }
...
interface IMigratorChef {
    // Perform LP token migration from legacy PanpipiSwap to PipiSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PanpipiSwap LP tokens.
    // PipiSwap must mint EXACTLY the same amount of PipiSwap LP tokens or
    // else something bad will happen. Traditional PanpipiSwap does not
    // do that so be careful!
    function migrate(IHRC20 token) external returns (IHRC20);
}

// MasterChef is the master of Pipi. He can make Pipi and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PIPI is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PIPIs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPipiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPipiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    PipiToken public pipi;
    // The SYRUP TOKEN!
    PipiBar public xPipi;
    // Dev address.
    address public devaddr;
    // PIPI tokens created per block.
    uint256 public pipiPerBlock;
    // Bonus muliplier for early pipi makers.
    ...
    constructor(
        ...
        PipiBar _xPipi,
        address _devaddr,
        uint256 _pipiPerBlock,
        uint256 _startBlock
    ) public {
        pipi = _pipi;
        xPipi = _xPipi;
        devaddr = _devaddr;
        pipiPerBlock = _pipiPerBlock;
    ...
    // Update the given pool's PIPI allocation point. Can only be called by the owner.
    ...
    // View function to see pending PIPIs on frontend.
    function pendingPipi(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 pipiReward = multiplier.mul(pipiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pipiReward = multiplier.mul(pipiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pipi.mint(devaddr, pipiReward.div(10));
        pipi.mint(address(xPipi), pipiReward);
        pool.accPipiPerShare = pool.accPipiPerShare.add(pipiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for PIPI allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'deposit PIPI by staking');
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safePipiTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw PIPI by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake PIPI tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        xPipi.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw PIPI tokens from STAKING.
    ...
    // Safe pipi transfer function, just in case if rounding error causes pool to not have enough PIPIs.
    function safePipiTransfer(address _to, uint256 _amount) internal {
        xPipi.safePipiTransfer(_to, _amount);
    }
